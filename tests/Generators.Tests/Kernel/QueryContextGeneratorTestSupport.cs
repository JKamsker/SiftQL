...
using SiftQL.Generators.QueryContexts;

namespace SiftQL.Generators.Tests;

internal static class QueryContextGeneratorTestSupport
{
    public static QueryContextGeneratorRun RunGenerator(params SyntaxTree[] trees)
    {
        ...
        return new QueryContextGeneratorRun(driver.GetRunResult(), outputCompilation, diagnostics);
    }
    ...
    public static GeneratorDriver CreateDriver(bool trackIncrementalSteps = false) =>
        CSharpGeneratorDriver.Create(
            generators: ImmutableArray.Create<ISourceGenerator>(
                new SiftQueryContextSourceGenerator().AsSourceGenerator()),
    ...
    public static CSharpCompilation CreateCompilation(params SyntaxTree[] trees)
    {
        ...
        AddReference(references, typeof(EventPipelineExpression).Assembly.Location);
    ...
    public static string GeneratedSource(QueryContextGeneratorRun run, string hintName) =>
        run.Result.Results[0].GeneratedSources.Single(source => source.HintName == hintName).SourceText.ToString();

    public static Assembly EmitAssembly(QueryContextGeneratorRun run)
    {
    ...
    public static void AssertNoCompilationErrors(QueryContextGeneratorRun run, string label)
    {
    ...
    public static void AssertHasDiagnostic(QueryContextGeneratorRun run, string diagnosticId) =>
        AssertEx.True(
...
internal sealed record QueryContextGeneratorRun(
    GeneratorDriverRunResult Result,
