...
internal static class EventProjectionSelectorTranslator
{
        ...
        ParameterExpression subject)
    {
        ...
        TranslateValue(StripConvert(body), subject, name: null, fields);
        if (fields.Count == 0)
        {
            throw new KernelExpressionException(
                "Projection selector must include at least one field.");
        }
        ...
        List<EventProjectionField> fields)
    {
        ...
        {
            TranslateMemberInit(initialized, subject, fields);
            return;
        }
