...
internal sealed class TypedRangeFieldSnapshot<TSubscription, TSubject>
    where TSubscription : class
{
    private readonly Func<object, decimal?> _accessor;
    private readonly decimal[] _lowerKeys;
    private readonly TypedSubscriptionEntry<TSubscription, TSubject>[] _lowerEntries;
    private readonly decimal[] _upperKeys;
    private readonly TypedSubscriptionEntry<TSubscription, TSubject>[] _upperEntries;

    public TypedRangeFieldSnapshot(
        Func<object, decimal?> accessor,
        decimal[] lowerKeys,
        TypedSubscriptionEntry<TSubscription, TSubject>[] lowerEntries,
        decimal[] upperKeys,
        TypedSubscriptionEntry<TSubscription, TSubject>[] upperEntries)
    {
        _accessor = accessor;
    ...
    public void AddCandidates(TSubject subject, List<TSubscription> candidates, HashSet<TSubscription> seen)
    {
        if (_accessor(subject!) is not { } x)
            return;
    ...
    public void AddMatches(TSubject subject, List<TSubscription> matches, HashSet<TSubscription> seen)
    {
        if (_accessor(subject!) is not { } x)
            return;
